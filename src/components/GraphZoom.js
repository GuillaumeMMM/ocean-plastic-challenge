...
class GraphZoom extends Component {
    ...
    zoomIn = (date) => {
        ...
        d3.select('.graph-axis-x').transition().duration(10000).call(d3.axisBottom(newX).tickSize(0));
        d3.select('.graph-axis-y').transition().duration(10000).call(d3.axisLeft(newY).tickSize(0));
        ...
        d3.select('.forecast-path').transition().duration(10000).style('stroke-width', (1 / ratioX) + 'px').attr("stroke-dasharray", 'none').attr("transform", translate + scale + translateInverse)
        // .on('end', this.splitLines.bind(this, this.state.x, this.state.y, newX, newY));

        d3.select('.now-path').transition().duration(10000).attr("transform", translate + scale + translateInverse).style('stroke-width', (1 / ratioX) + 'px');
    }

    splitLines = (x, y, newX, newY, date) => {
        const forecastPathClone = d3.select('.forecast-path').clone();
        ...
        d3.select('.forecast-path').transition().duration(1000)
